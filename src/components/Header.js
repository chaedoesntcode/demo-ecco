...
const Header = ({activeTab, handleTabClick}) => {
  ...
  <header className='text-[#282828]'>
    <div className=' bg-[#282828] text-white text-center py-1 uppercase text-sm'>Free Returns & Shipping on All Orders</div>
    <div className={`${isActive ? 'border-b shadow-sm border-[#f2f2f2] fixed top-0' : 'absolute'} bg-white transition-all z-40 w-full`}>
      <div className='px-5 md:px-10 h-[100px] flex justify-between items-center'>
        ...
        <div className='flex justify-center items-center gap-2 md:gap-4'>
          <div  onClick={() => setIsOpen(!isOpen)} className='relative cursor-pointer hover:bg-[#f2f2f2] p-2 rounded-md transition'>
            <AiOutlineShopping className='text-4xl'/>
            <div className='absolute top-2 right-[2px] text-xs w-[20px] h-[20px] flex justify-center items-center bg-[#282828] text-white rounded-full'>{itemQty}</div>
          </div>
